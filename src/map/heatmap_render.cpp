...
HeatmapRenderer::HeatmapRenderer()      // конструктор для выделения памяти на пиксели будещей карты
{
    image_data.resize(w * h * channels);
}

Color HeatmapRenderer::GradientColor(Color c1, Color c2, double ratio)  // линейная интерполяция цвета
{
    return {
...
double HeatmapRenderer::CalculateDistance(double lat1,  // функция для вычисления расстояния между двумя точками
                                          double lon1,
                                          double lat2,
                                          double lon2)
{
...
void HeatmapRenderer::UpdateData(const std::vector<double> &lats,   // обновление генерации карты
...
void HeatmapRenderer::StartGeneration()             // начало генерации путем создания отдельного потока
{
...
void HeatmapRenderer::GenerateTask()                // сама генерация тепловой карты
{
...
void HeatmapRenderer::SaveToFile(const std::string &path) // экспорт в PNG
{
    stbi_write_png(
        path.c_str(),
        w,
...
void HeatmapRenderer::ProcessGPUUpload() // загрузка в OpenGL
{
    if (!m_needsUpload)
        return;
    if (m_textureID == 0)
...
void HeatmapRenderer::UIDrawOverlay()  // рисует текстуру как изображение в координатах графика
{
    if (m_textureID == 0)
        return;
